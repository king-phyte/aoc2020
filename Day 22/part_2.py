...
The winner keeps both cards, placing them on the bottom of their own deck so that the winner's card is
...
(For example, if player 1 drew the 3 card, their deck in the sub-game would be copies of the next three
cards in their deck.)
...
def play_game(games_played: set, player1: Sequence[int], player2: Sequence[int]) -> Tuple[List[int], List[int]]:
    while player1 and player2:
        ...
        player_1_wins, _ = play_game(set(), player1[:a], player2[:b])
...
def main():
    ...
    print(sum_winner_score(winner))  # Answer = 32665
